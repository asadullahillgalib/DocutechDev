...
@Named
@Slf4j
public class ScannedItextCtechInvoiceUploadService extends AbstractItextScannedInvoiceUploadService {
	@Autowired
	...
	@Value("${import.template.ctechTemplate-path}")
	private String ctechTemplatePath;
	...
	private ParsingResult getResultFromPdf(File file) throws IOException {
		...
		Template template;
		try {
			template = Pdf2DataExtractor.parseTemplateFromPDF(ctechTemplatePath);
	...
	private DocumentWrapper convertInvoiceToDocumentReq(ParsingResult inv) {
		if (inv == null)
			return null;
		try {
			DocumentWrapper ob = DocumentWrapper.createWithName("C-Tech London Ltd", "CTEC");

			// Setting attributes of the document wrapper object
			ob.setTitle("C-Tech London Ltd");
			...
			ob.addAttribute("contact_number", valueToStringOrEmpty(inv, "contactNumber"));
			ob.addAttribute("customer_address", valueToStringOrEmpty(inv, "customerAddress"));
			ob.addAttribute("email_address", valueToStringOrEmpty(inv, "emailAddress"));
			ob.addAttribute("invoice_date", valueToStringOrEmpty(inv, "invoiceDate"));
			ob.addAttribute("payment_details", valueToStringOrEmpty(inv, "paymentDetails"));
			...
			ob.addAttribute("total_amount_due", valueToStringOrEmpty(inv, "totalAmountDue"));
	...
	public void uploadDocument(List<File> attachments, boolean flag) throws IOException {
		...
		if (null != result) {
			...
			tmplt.setReceivedDocId(tmplt.getClientName() + "_" + valueToStringOrEmpty(result, "referenceNo"));
			...
			if (result.getErrorsNumber() >= 3) {
				...
				//document.setSupplierNumber(Integer.parseInt(valueToStringOrEmpty(result, "companyRegNo")));
				...
				if (valueToStringOrEmpty(result, "totalAmountDue").isEmpty()) {
					document.setNetInvoiceTotal(0.0);
				} else {
					document.setNetInvoiceTotal(Double.parseDouble(valueToStringOrEmpty(result, "totalAmountDue").replaceAll(",", "")));
				}
				...
				document.setTotalBeforeVat(Double.parseDouble(valueToStringOrEmpty(result, "totalAmountDue").replaceAll(",", "")));
				document.setTotalPrice(Double.parseDouble(valueToStringOrEmpty(result, "totalAmountDue").replaceAll(",", "")));
				...
				try {
					document.setDespatchDate(
							new SimpleDateFormat("yyyy/MM/dd").parse(valueToDateOrDefault(result, "despatchDate")));

...
public class DocDetailsService extends AbstractService<DocDetails> {
	...
	private List<DocDetails> handleDocDetails(String action, DocDetails docDetails, Message<List<DocDetails>> msg)
			throws Exception {
		log.debug("executing...{}", action);

//		DocDetails details = msg.getPayload().get(0);
//
//		if (null == docDetails) {
//			docDetails = details;
//		}
		...
		try {
			...
			jdbcResult = getJdbcService().executeSP(action, SPName.DOCUTECH.toString(),
					SPName.ACT_DOC_DETAILS.toString(), spArgsMap, jdbcResult);

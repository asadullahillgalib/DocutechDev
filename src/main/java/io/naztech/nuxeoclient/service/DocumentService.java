...
public class DocumentService extends AbstractService<Document> {
	...
	public Message<?> serviceSingle(Message msg) throws Exception {
		...
		try {
			...
			} else if (actionType.equals(ActionType.NEW_CUSTOMER.toString()) || actionType.equals(ActionType.UPDATE_CUSTOMER.toString())
...
//			}
	 ...
	 */
//	private List<Document> handleDocDetails(String action, Document doc) throws Exception {
...
//					.getJdbcStoredProcedure(SPName.DOCUTECH.toString(), SPName.ACT_DOC_DETAILS.toString());
...
//
//			jdbcResult = getJdbcService().executeSP(action, SPName.DOCUTECH.toString(), SPName.ACT_DOC_DETAILS.toString(),

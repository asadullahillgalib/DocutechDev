...
public class TemplateService extends AbstractService<Template> {
	...
	private List<Template> handleTemplate(String action, Template tmplt, Message<List<Template>> msg) throws Exception {
		...
		//Template template = msg.getPayload().get(0);
...
//		}
		...
		try {
			...
			JdbcStoredProcedure jdbcStoredProcedure = getJdbcService()
					.getJdbcStoredProcedure(SPName.DOCUTECH.toString(), SPName.ACT_TEMPLATE.toString());
			...
			jdbcResult = getJdbcService().executeSP(action, SPName.DOCUTECH.toString(), SPName.ACT_TEMPLATE.toString(),
	 ...
	 * @desc:
	 */
	private List<Template> handleTemplateException(String action, Template tmplt, Message<List<Template>> msg)
			throws Exception {
		...
		try {
			...
			jdbcResult = getJdbcService().executeSP(action, SPName.DOCUTECH.toString(),
					SPName.ACT_TEMPLATE_EXCEPTION.toString(), spArgsMap, jdbcResult);
